...
class KeychipResponsePacketProc extends AuthPacketProc
{
    ...
    function proc() {
        ...
        // 署名付与
        $private_key_file = $this->ini["private_key_file"];
        $certificate_file = $this->ini["certificate_file"];
        $cms_tmp_dir = $this->ini["cms_tmp_dir"];

        $message = $this->create_message($result, $keychip_id );
        $verify_message = $this->add_sign($message,$private_key_file, $certificate_file, $cms_tmp_dir);
        if ( ! $verify_message ) {

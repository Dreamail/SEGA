...
class GameAttribute
{
     ...
     */
    public static function findByPk($titleId, $titleVer, $countryCode)
    {
        // 戻り値
        $result = array();

        // ゲームID、ゲームVer.、国コードが空の場合は取得しない
        if (empty($titleId) || empty($titleVer) || empty($countryCode)) {
            return $result;
        }
        ...
        $params = array($titleId, $titleVer, $countryCode);
        LogUtil::sqlLog("[ゲーム属性情報:検索]"
            . " query=" . $query
            . " param=(" . $titleId . ", " . $titleVer . ", " . $countryCode . ")");
        try {

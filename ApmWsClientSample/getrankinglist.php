...
if ($_POST != null) {
    ...
    $pass = $_POST['pass'];
    $rankingIds = $_POST['rankingIds'];
    if (isset($_POST['current']) && $_POST['current'] == "true") {
        $current = true;
    }

    // 複数ランキング一括取得リクエスト用のJSONを生成するために連想配列を定義します
    $param = array(
        "pass" => $pass,    // アクセスパスワード
        "param" => array(
            "rankingIds" => explode(",", $rankingIds),  // ランキングID（配列を指定します）
    ...
    <body>
        ...
        <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
            <dl>
                ...
                <dt>ランキングIDリスト</dt>
                <dd><input type="text" name="rankingIds" value="<?php echo $rankingIds; ?>"></dd>
                <dt>現ランキング対象</dt>
                <dd>
                <?php if ($current): ?>
